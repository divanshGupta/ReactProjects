...
// Route for getting all sponsors from the database
router.get('/all', async (request, response) => {
    try {
        const sponsors = await Sponsor.find({});
        return response.status(200).json({
...
// Route for getting a specific sponsor using its id
router.get('/:id', async (request, response) => {
    try {
        const { id } = request.params;
        if (!id) {
...
router.post('/', async (request, response) => {
    try {
        if (
            ...
            return response.status(400).send({
                message: 'Send all required fields and array: orgName, orgType, industry, budget, socials',
            });
        }
...
router.put('/:id', async (request, response) => {
    try {
        if (
            ...
            return response.status(400).send({
                message: 'Send all required fields: orgName, orgType, industry, budget, socials',
            });
        }
